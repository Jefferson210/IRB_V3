class TwoOffspringsController < ApplicationController
            ...
            if @two_offspring.save
                if params[:images]              
                    params[:images].each { |image|
                      @two_offspring.two_offspring_pictures.create(picture: image)
                    }
                end  
    ...
    def two_offspring_params
        params.require(:two_offspring).permit(:one_offspring_id, :individual, :color, :status, :headSize, :flowering, :numPetals, :form, :PictureId)
    end
end
